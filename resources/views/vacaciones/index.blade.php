<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Vacaciones del empleado : <a class="text-red-500 uppercase underline" href="{{route('empleados.show',$empleado)}}"> {{$empleado->nombre}} {{$empleado->apellidos}}
            </a>
        </h2>
    ...
    <div class="pl-88 mt-4 flex justify-start">
        <x-boton2 tipo="link" class="bg-green-600 hover:bg-green-700 flex justify-around w-48 h-9" direccion="{{ route('vacaciones.create', $empleado) }}">
            <x-slot name="boton">
                Añadir vacaciones
                                    ...
                                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                                        <table class="min-w-full divide-y divide-gray-200 w-full ">
                                            ...
                                            @foreach ($empleado->vacaciones as $vacacion)
                                                <tr class="hover:bg-green-200" onclick="detalles('{{ route('vacaciones.show', ['empleado' => $empleado, 'vacacion' => $vacacion]) }}', event)">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-start">
                                                        <x-boton2 tipo="link" class="bg-yellow-400 hover:bg-yellow-600 mr-4 w-14 h-14" direccion="{{ route('vacaciones.edit', ['empleado' => $empleado, 'vacacion' => $vacacion]) }}">
                                                            <x-slot name="boton">
