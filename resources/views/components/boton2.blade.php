...
@if ($tipo == "linkConAsset")
    <div {{ $attributes->merge(['class' => 'flex  space-x-2 justify-center items-center text-white px-4 py-2 rounded-md text-1xl font-medium transition duration-300'])}}>
        <a href="{{ asset("/storage/$direccion") }}" target="_blank">{{ $boton }}</a>
    </div>
@endif

@if ($tipo == "input")
    <div {{ $attributes->merge(['class' => ' space-x-2 flex justify-center text-white px-4 py-2 rounded-md text-1xl font-medium transition duration-300'])}}>
        <button >
            {{ $boton }}
        </button>
    </div>
