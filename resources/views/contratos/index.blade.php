<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            Cliente : <a class="text-red-500 uppercase underline" href="{{route('clientes.show', $cliente)}}"> {{$cliente->nombre}} {{$cliente->apellidos}}</a>
        </h2>
    </x-slot>
                                            ...
                                            @foreach ($cliente->contratos as $contrato)
                                                <tr class="hover:bg-green-200" onclick="detalles('{{ route('contratos.show', ['cliente' => $cliente->id, 'contrato' => $contrato->id]) }}', event)">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-start">
                                                        <x-boton2 tipo="link" class="bg-yellow-400 hover:bg-yellow-600 mr-4 w-16" direccion="{{ route('contratos.edit', ['cliente' => $cliente->id, 'contrato' => $contrato->id]) }}">
                                                            <x-slot name="boton">
